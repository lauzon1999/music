...
mainAudio.addEventListener("ended", () => {
  ...
  switch (getText) {
    ...
    case "shuffle":
      ...
      window.CP.exitedLoop(0);musicIndex = randIndex; //passing randomIndex to musicIndex
      loadMusic(musicIndex);
      playMusic();
      playingSong();
      break;}
...
for (let i = 0; i < allMusic.length; i++) {if (window.CP.shouldStopExecution(1)) break;
...
window.CP.exitedLoop(1);function playingSong() {
  const allLiTag = ulTag.querySelectorAll("li");

  for (let j = 0; j < allLiTag.length; j++) {if (window.CP.shouldStopExecution(2)) break;
